    ...
    add: (newData: CashFlow) => {
      store.update((data) => {
        ...
        while (data[keys[0]] !== undefined) {
          if (
            cashFlowComparatorOrder(newData, data[keys[0]]) &&
            flagIsNotAdded
          ) {
    ...
    swap: (i: string) => {
      store.update((d) => {
        ...
        while (d[keys[0]] !== undefined) {
          ...
          const foundWhereAdd =
            cashFlowComparatorOrder(reorderedData, d[keys[0]]) &&
            flagIsNotAdded;

          if (foundWhereRemove && foundWhereAdd) {
            ...
            if (foundWhereRemove) {
              // Found where remove data
              keys[0]++;
            } else if (foundWhereAdd) {
              // Found where add data
              sortedData[keys[1]] = reorderedData;
              flagIsNotAdded = false;
              keys[1]++;
            } else {
              // Data not found at this iteration for add or remove
              sortedData[keys[1]] = d[keys[0]];
              keys[0]++;
              keys[1]++;
            }
