<script lang="ts">
  ...
  export let validateDelete: (arg0: any, arg1: string) => boolean = (_, __) =>
    true;
  ...
  function computeAndFormatTotal<T>(
    column: GenericColumn<T>,
    key: string,
    data: T,
    indexes: string[],
  ...
  function applyFilter(entries: [string, any][], filters: {}): [string, any][] {
    const filteredEntries: [string, any][] = entries.filter(([_, value]) => {
...
<table class="striped">
    ...
    <tr>
      {#each Object.entries(columns) as [key, item]}
        <th>
          ...
          {#if showFilters[key]}
            <input
              on:input={(e) => onFilterClick(e, key)}
              value={filters[key] || ""}
  ...
  <tbody>
    <tr>
      ...
      <td>
        <label
          class="button outline icon-only pull-right"
