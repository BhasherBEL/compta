<script lang="ts">
  ...
  const totalRow: { [key in keyof Account]: GenericColumn<Account> } = {
    ...
    initial_money: {
      ...
      required: false,
      compute: (_, __, ___) =>
        sum(Object.values($accounts).map((a) => a.initial_money)),
      format: formatMoney,
    },
    income: {
      name: text.income,
    ...
    profit: {
      ...
      required: false,
      compute: (_, __, ___) =>
        sum(Object.values($cashFlows).map((k) => k.amount)),
      format: (amount, __, _) => {
        return `<span
                            style="background-color: ${picker(
                              amount,
                              "#dfffdf",
                              "#fed4d4"
                            )}"
    ...
    current_money: {
      ...
      required: false,
      compute: (_, __, ___) =>
        sum(Object.values($accounts).map((a) => a.current_money)),
      format: (amount, __, _) => {
        let all_initial_money = sum(
          Object.values($accounts).map((a) => a.initial_money)
        );
        let calculated =
          sum(Object.values($cashFlows).map((k) => k.amount)) +
          all_initial_money;
        let okay = Math.round(calculated * 100) == Math.round(amount * 100);
        return `<span class="${okay ? "" : "text-error"}">${formatMoney(
          amount
        )} ${
          okay ? "" : `(${text.diff_of} ${formatMoney(calculated - amount)})`
        }</span>`;
      },
    },
  };
  ...
  const columns: { [key in keyof Account]: GenericColumn<Account> } = {
    ...
    current_money: {
      ...
      format: (amount, account, id) => {
        ...
        let okay =
          Math.round(calculated * 100) ==
          Math.round(account.current_money * 100);
        return `<span class="${okay ? "" : "text-error"}">${formatMoney(
          amount
        )} ${
          okay
            ? ""
            : `(${text.diff_of} ${formatMoney(
                calculated - account.current_money
              )})`
        }</span>`;
      },
    },
