<script lang="ts">
  ...
  const columns: { [key in keyof CashFlow]: GenericColumn<CashFlow> } = {
    ...
    account: {
      name: text.account,
      type: "select",
      suggestions: Object.entries($accounts).map(
    ...
    in_out: {
      ...
      type: "string",
      required: false,
      compute: (_, cashFlow, __) =>
        formatColor(
          cashFlow.amount,
          picker(cashFlow.amount, text.income, text.expense)
        ),
    },
  };
  ...
  const unsubscribe = cashFlows.subscribe((flows) => {
    ...
    for (const key of trackedKeys) {
      columns[key].suggestions = columns[key].suggestions
        .concat(Object.values(flows).map((flow) => flow[key]))
        .filter(unique);
    }
  });
